...
async function uploadBlob(blob, filename) {
    ...
    try {
        ...
        for (const uploaded_file of uploads) {
            ...
            let liinner = `<a class="thumb-link" href="${uploaded_file.url}" target="_blank">` + (uploaded_file.name.toLowerCase().endsWith(".pdf") ? '<img class="thumb" src="/static/pdf-icon.svg" alt="PDF" style="max-width:48px; border:none;">' : `<img class="thumb" src="${uploaded_file.url}" loading="lazy" alt="${uploaded_file.name}">`) + `</a><span class="copy-name" data-url="${uploaded_file.url}" onclick=\"copyURLToClipboard(this)\" title="copy link">🔗 ${uploaded_file.name}</span> &nbsp;— <small>just now</small>`;
            li.innerHTML = liinner;
            ul.insertBefore(li, ul.firstChild);
            the_urls.push(uploaded_file.url);
        }
        setClipboard(the_urls.join("\n"));
        return data;
    } catch (err) {
        console.error(err);
...
// expose for template init
window.initImagesPage = initImagesPage;
